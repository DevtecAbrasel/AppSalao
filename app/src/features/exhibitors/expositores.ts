 ...
 *
 * Os 22 nomes que antes não tinham posição eram, quase todos, os moradores
 * dos dois ESTANDES COMPARTILHADOS — e é por isso que o impresso não dizia
 * onde ficam: o desenho não separa as empresas lá dentro. Agora eles apontam
 * para o estande (campo `estande`) e herdam a posição DELE, de modo que a
 * coordenada continua existindo num lugar só: `estandesCompartilhados.ts`.
 */
import { ESTANDES_COMPARTILHADOS } from "../map/estandesCompartilhados";
...
export interface Expositor {
  ...
  nomeNaPlanta?: string;
  /**
   * Chave do estande compartilhado em que a empresa está, quando é o caso.
   * A posição vem de lá — não se escreve `x`/`y` junto com isto.
   */
  estande?: string;
}

// O que o participante lê na planta em cima desses stands é a palavra
// COMPARTILHADO, não o nome da empresa. Dizer isso na lista evita a busca
// frustrada por um letreiro que não existe.
const ROTULO_COMPARTILHADO = "Estande compartilhado";

const LISTA: Expositor[] = [
  ...
  // Artesian, LUCY e PaySales não constavam da lista impressa — chegaram com
  // os estandes compartilhados enviados pela organização.
  { key: "artesian", nome: "Artesian", estande: "compartilhado-caex" },
  { key: "banco-do-brasil-cielo", nome: "Banco do Brasil Cielo", x: 0.778, y: 0.522 },
  { key: "bares-sp", nome: "Bares SP", estande: "compartilhado-arena-sebrae" },
  { key: "bidfood", nome: "Bidfood", x: 0.788, y: 0.269 },
  { key: "chef-ai", nome: "Chef.ai", estande: "compartilhado-arena-sebrae" },
  ...
  { key: "conta-clara", nome: "Conta Clara", estande: "compartilhado-caex" },
  { key: "control-bpo", nome: "Control BPO", x: 0.686, y: 0.102 },
  { key: "cotacompras", nome: "Cotacompras", estande: "compartilhado-arena-sebrae" },
  { key: "deliveryvip", nome: "DeliveryVIP", estande: "compartilhado-arena-sebrae" },
  { key: "dpen", nome: "DPEN", estande: "compartilhado-caex" },
  { key: "figa", nome: "Figa", x: 0.862, y: 0.79 },
  { key: "fixe-crm-fidelidade", nome: "Fixe CRM & Fidelidade", estande: "compartilhado-arena-sebrae" },
  { key: "fixee", nome: "Fixee", estande: "compartilhado-arena-sebrae" },
  { key: "frescatto", nome: "Frescatto", x: 0.789, y: 0.198 },
  { key: "gelopar", nome: "Gelopar", x: 0.885, y: 0.757 },
  { key: "gerenciar", nome: "Gerenciar", estande: "compartilhado-arena-sebrae" },
  ...
  { key: "infinite", nome: "Infinite", estande: "compartilhado-arena-sebrae" },
  { key: "itw", nome: "ITW", x: 0.754, y: 0.412 },
  { key: "keeta", nome: "Keeta", x: 0.52, y: 0.496 },
  { key: "kikkoman", nome: "Kikkoman", estande: "compartilhado-arena-sebrae" },
  { key: "lucy", nome: "LUCY", estande: "compartilhado-caex" },
  ...
  { key: "moncoc", nome: "Moncoc", estande: "compartilhado-caex" },
  { key: "natural-bot", nome: "Natural Bot", estande: "compartilhado-arena-sebrae" },
  ...
  { key: "nsf", nome: "NSF", estande: "compartilhado-arena-sebrae" },
  { key: "olaclick", nome: "Olaclick", estande: "compartilhado-arena-sebrae" },
  { key: "pagbank", nome: "PagBank", x: 0.805, y: 0.412 },
  { key: "paysales", nome: "PaySales", estande: "compartilhado-arena-sebrae" },
  ...
  { key: "processoalerta", nome: "ProcessoAlerta", estande: "compartilhado-caex" },
  ...
  { key: "strikes-solucoes", nome: "Strikes Soluções", estande: "compartilhado-arena-sebrae" },
  { key: "super-gestor", nome: "Super Gestor", estande: "compartilhado-arena-sebrae" },
  { key: "svb", nome: "SVB", estande: "compartilhado-arena-sebrae" },
...
// Quem mora num estande compartilhado herda a posição DELE. A coordenada
// continua escrita num lugar só (`estandesCompartilhados.ts`): mudou lá, muda
// aqui. O `nomeNaPlanta` vira "Estande compartilhado" porque é isso, e não o
// nome da empresa, que está escrito no chão do salão.
export const EXPOSITORES: Expositor[] = LISTA.map((e) => {
  if (!e.estande) return e;
  const estande = ESTANDES_COMPARTILHADOS.find((s) => s.key === e.estande);
  return estande
    ? { ...e, x: estande.x, y: estande.y, nomeNaPlanta: ROTULO_COMPARTILHADO }
    : e;
});
